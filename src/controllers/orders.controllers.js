...
export const createOrder = async( req, res) => {
    ...
    .query('INSERT INTO H2O.ORDERS ( idClient, dateOrder, idAddress, idOrderStatus, total, idTypePayment) VALUES (@idClient, GETDATE() , @idAddress, 1,@total, @idTypePayment); SELECT SCOPE_IDENTITY() AS idOrder;');
    ...
    for (const item of cartItems) {
        await pool
            ...
            .input("subtotal", sql.Decimal, item.quantity * item.priceProduct)
            .query(`
...
export const ordersStatus = async (req, res) => {
    try {
        ...
        if (result.recordset.length === 0) {
            return res.status(404).json({ message: "No orders found for the given date and status" });
        }
